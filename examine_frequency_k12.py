...
if __name__ == "__main__":
    ...
    for k12 in range(1, 80, 1):
        ...
        py.savefig("".join(['exploration/plots/k12-', str(k12), '-average.png']))
        ...
        Pxx.plot(marker='o')
        # py.show()
        py.savefig("".join(['exploration/plots/k12-', str(k12), '-spectrum.png']))

        try:
            with open('exploration/k12-dominant-freq.pickle', 'rb') as f:
                freqs = pickle.load(f)
        except (FileNotFoundError, EOFError):
            freqs = dict()

        max_index, _ = max(enumerate(Pxx.psd[5:]), key=operator.itemgetter(1))
        freqs[k12] = Pxx.frequencies()[max_index+5]

        print('For k12 = ', k12, ' dominant frequency is ', freqs[k12])

        with open('exploration/k12-dominant-freq.pickle', 'wb') as f:
            pickle.dump(freqs, f)
