...
if __name__ == "__main__":
    ...
    for k21 in range(1, 80, 1):
        ...
        py.savefig("".join(['exploration/plots/k21-', str(k21), '-average.png']))
        ...
        Pxx.plot(marker='o')
        # py.show()
        py.savefig("".join(['exploration/plots/k21-', str(k21), '-spectrum.png']))

        try:
            with open('exploration/k21-dominant-freq.pickle', 'rb') as f:
                freqs = pickle.load(f)
        except (FileNotFoundError, EOFError):
            freqs = dict()

        max_index, _ = max(enumerate(Pxx.psd[5:]), key=operator.itemgetter(1))
        freqs[k21] = Pxx.frequencies()[max_index+5]

        with open('exploration/k21-dominant-freq.pickle', 'wb') as f:
            pickle.dump(freqs, f)
